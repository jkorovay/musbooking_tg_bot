...
> Это портфолио-сборка для демонстрации кода. Идентификаторы студии (`ROOM_ID`, `OPTION_ID`), токен бота и список разрешённых Telegram ID не входят в репозиторий — их вписывают в `.env` (шаблон в `.env.example`). Сами идентификаторы не секретные — их видно в публичном виджете MusBooking (через инструменты разработчика в браузере). Доступ к живому боту закрыт списком `ALLOWED_IDS`: ссылку на бота можно дать кому угодно, но пользоваться им смогут только те, чей Telegram ID добавлен в этот список.
